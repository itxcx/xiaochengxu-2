   ...
   getBuy: function (page, catid, areaid, keyword) {
      ...
      data.getBuy(function (data) {
            ...
            for (var i = 0; i < data.data.length; i++) {
               arr.push({ title: data.data[i].title, areaname: data.data[i].areaname, company: data.data[i].company, catname: data.data[i].catname, edittime: new Date(data.data[i].addtime * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ') })
            }
            that.setData({
               zixun: arr,
               content: data.data,
